...
# For the main effect of ‘Diagnosis’, a minimum effect size of 0.3 was specified, based on a study comparing children with ASD and TD children (Venuti, de Falco, Esposito, Zaninelli, & Bornstein, 2012). This study showed an average difference of 0.4, which is more than what we have found in our data (0.23). Therefore, we have chosen to be less conservative in this specified effect, setting it to 0.3. According to this, the effect size found in our data (0.23) is not big enough to be relevant (these children were matched on language abilities - in the beginning of the study, and this might be the reason why the effect does not appear that big in the data. It would probably have been bigger if the children had been matched on age).
...
fixef(model_extended)["DiagnosisTD"] <- 0.3
...
pc_Diagnosis2 <- powerCurve(model_extended, test = fixed("Diagnosis"), along = "Child.ID", nsim = 100, seed = 1)
print(pc_Diagnosis2)
plot(pc_Diagnosis2)
# In order to find a main effect of "Diagnosis" with a very conservative approach (effect size of 0.3), one would need around 196 participants (resulting in power of .80)
