...
fixef(model0)["DiagnosisTD"] # main effect of -0.26
...
powerSim(model0, test = fixed("Diagnosis"), nsim = 100) # 89% power for detecting effect of diagnosis (nsim = 100)
...
powerSim(model, test = fixed("Visit:Diagnosis:verbalIQ1"), nsim=100) # 97% power for detecting the three-way interaction effect (nsim = 100)


# Report the power analysis and comment on what you can (or cannot) use its estimates for.

# The interaction of Visit * Diagnosis * VerbalIQ1
# Calculating power based on 1000 simulations gives a beta value of 0.97. This means, that an interaction effect of this size (0.02) is easily detected with a sample like this. It might even be an overpowered study.
# COMMENT ON MAIN EFFECTS


# Test how many participants you would have to have to replicate the findings (assuming the findings are correct)

# We assume that we do not need to extend our simulation, since the powers of the different effects already are above 0.80. It is rather a question about not overpowering the study. We do not need this many participants to detect these effects - neither main effects nor interaction effect.

# Main effects
pc_Visit <- powerCurve(model0, test = fixed("Visit"), along = "Child.ID", nsim = 100)
print(pc_Visit)
plot(pc_Visit)
# In order to replicate the findings of a main effect of Visit, one would need 10 participants (resulting in a beta-value/power of .87)

pc_Diagnosis <- powerCurve(model0, test = fixed("Diagnosis"), along = "Child.ID", nsim = 100)
print(pc_Diagnosis)
plot(pc_Diagnosis)
# In order to replicate the findings of a main effect of Diagnosis, one would not really need that many participants. Even with as few as 3 participants, the power is estimated to 0.92.

pc_VerbalIQ <- powerCurve(model0, test = fixed("verbalIQ1"), along = "Child.ID", nsim = 100)
print(pc_VerbalIQ)
plot(pc_VerbalIQ)
# In order to replicate the findings of a main effect of verbal IQ, one would need aroung xx participants (resulting in a beta-value/power of .xx)


pc_inter <- powerCurve(model, test = fixed("Visit:Diagnosis:verbalIQ1"), along = "Child.ID", nsim = 100)
print(pc_inter)
plot(pc_inter)
# In order to replicate the findings of a three-way interaction effect, one would need 31 participants (resulting in a beta-value/power of .8)
...
fixef(model)["Visit:DiagnosisTD:verbalIQ1"] <- 0.2
